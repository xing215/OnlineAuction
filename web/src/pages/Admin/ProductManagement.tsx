...
const ProductManagement = () => {
  ...
  const TabButton = ({ label, value }: { label: string, value: string }) => (
    <button
      ...
      className={`relative pb-3 px-1 text-sm font-medium transition-colors ${status === value ? 'text-[#D9A52A] border-b-2 border-[#D9A52A]' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300 border-b-2 border-transparent'}`}
    >
      {label}
    </button>
  );
    ...
    <div className="min-h-screen bg-[#F8F9FA] p-6 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Dữ liệu từ Server</p>
        </div>
