...
export const useUserRatingsByUserId = (userId: string | null) => {
    const { token } = useUser();
    ...
    useEffect(() => {
        ...
        const fetchRatings = async () => {
            ...
            setError(null);
            try {
                // Fetch orders where the specified user is either seller or winner
                ...
                const response = await fetch(
                    apiUrl(`/api/orders/my-orders?user=${userId}`),
                    { headers }
                );
                ...
                orders.forEach((order) => {
                    // Nếu userId là seller và có winner_feedback
                    if (order.seller === userId && order.winner_feedback) {
                        const winnerName =
                    ...
                    // Nếu userId là winner và có seller_feedback
                    if (order.winner === userId && order.seller_feedback) {
                        const sellerName =
